...
<head>
    ...
    <style>
        ...
        #eegCanvas { height: 100px; }
        #hypnogramCanvas { height: 120px; margin-bottom: 15px;}
        ...
        button:hover { background: #318bf8; }
...
<body>
    ...
    <div class="container">
        ...
        <div class="card">
            ...
            <div style="text-align: center;">
                <div id="stageBadge" class="stage-badge stage-WAKE">WAKE</div>
            </div>
                ...
                <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(250px, 1fr)); gap: 20px;">
                    <div>
                        <div class="form-group">
                            <label>Alarm Window Start (HH:MM)</label>
                            <div class="time-inputs">
                                <input type="number" id="cfg_minH" min="0" max="23"> : <input type="number" id="cfg_minM" min="0" max="59">
                            </div>
                        </div>
                        <div class="form-group">
                            <label>Alarm Window End (HH:MM)</label>
                            <div class="time-inputs">
                                <input type="number" id="cfg_maxH" min="0" max="23"> : <input type="number" id="cfg_maxM" min="0" max="59">
                            </div>
                        </div>
                        <div class="form-group">
                            <label>Alarm Volume (0-30)</label>
                            <input type="number" id="cfg_vol" min="0" max="30">
                        </div>
                    </div>
                    <div>
                        ...
                        <div class="form-group">
                            <label><input type="checkbox" id="cfg_ap"> Access Point Mode</label>
                        </div>
                        <div class="form-group">
                            <label><input type="checkbox" id="cfg_raw"> Save Raw EEG to SD (Warning: Large files)</label>
                        </div>
                    </div>
                </div>
    ...
    <script>
        ...
        setInterval(() => {
            const d = new Date();
            document.getElementById('timeDisplay').textContent = d.toLocaleTimeString('en-US', { hour12: false });
        }, 1000);

        // EEG Canvas
        const eegCanvas = document.getElementById('eegCanvas');
        const ctxEeg = eegCanvas.getContext('2d');
        const eegData = new Array(eegCanvas.width).fill(0);
        let eegIdx = 0;
        
        function drawEeg() {
            ctxEeg.clearRect(0, 0, eegCanvas.width, eegCanvas.height);
            ctxEeg.beginPath();
            ctxEeg.strokeStyle = '#3fb950';
            ctxEeg.lineWidth = 2;
            
            const mid = eegCanvas.height / 2;
            
            for (let i=0; i<eegCanvas.width; i++) {
                const idx = (eegIdx + i) % eegCanvas.width;
                const val = eegData[idx];
                const y = mid - (val * (eegCanvas.height/2) / 500);
                
                if (i===0) ctxEeg.moveTo(i, y);
                else ctxEeg.lineTo(i, y);
            }
            ctxEeg.stroke();
            ...
            ws.onmessage = (e) => {
                try {
                    ...
                    if (data.eeg !== undefined) {
                        eegData[eegIdx] = data.eeg;
                        eegIdx = (eegIdx + 1) % eegCanvas.width;
                    }
                    if (data.stage !== undefined) {
                        const s = data.stage;
                        const sName = (s >= 0 && s <= 3) ? stages[s] : "UNKNOWN";
                        const badge = document.getElementById('stageBadge');
                        badge.textContent = sName;
                        badge.className = 'stage-badge stage-' + sName;
                    }
                    if (data.conf) {
                        document.getElementById('c-wake').style.width = (data.conf[0]*100) + '%';
                        document.getElementById('c-light').style.width = (data.conf[1]*100) + '%';
                        document.getElementById('c-deep').style.width = (data.conf[2]*100) + '%';
                        document.getElementById('c-rem').style.width = (data.conf[3]*100) + '%';
                    }
                    if (data.bands) {
                        const updateBand = (id, val) => {
                            const p = Math.min(100, Math.max(0, val * 100));
                            document.getElementById('b-'+id).style.width = p + '%';
                            document.getElementById('p-'+id).textContent = Math.round(p) + '%';
                        };
                        updateBand('delta', data.bands.d);
                        updateBand('theta', data.bands.t);
                        updateBand('alpha', data.bands.a);
                        updateBand('beta', data.bands.b);
                        updateBand('gamma', data.bands.g);
                    }
                } catch (e) {}
            };
        }
        ...
        fetch('/api/config')
            .then(res => res.json())
            .then(cfg => {
                document.getElementById('cfg_minH').value = cfg.minWakeHour;
                document.getElementById('cfg_minM').value = cfg.minWakeMinute;
                document.getElementById('cfg_maxH').value = cfg.maxWakeHour;
                document.getElementById('cfg_maxM').value = cfg.maxWakeMinute;
                document.getElementById('cfg_vol').value = cfg.alarmVolume;
                document.getElementById('cfg_ssid').value = cfg.wifiSSID;
                document.getElementById('cfg_pass').value = cfg.wifiPassword;
                document.getElementById('cfg_ap').checked = cfg.apMode;
                document.getElementById('cfg_raw').checked = cfg.saveRawEEG;
            }).catch(e => console.log('Config fetch err', e));
        ...
        fetch('/api/sleep')
            ...
            .then(epochs => {
                ...
                const w = canvas.width / epochs.length;
                let wCnt=0, lCnt=0, dCnt=0, rCnt=0;
                ...
                epochs.forEach((ep, i) => {
                    ...
                    if (s>=0 && s<=3) {
                        ...
                        if(s===0) wCnt++; else if(s===1) lCnt++; else if(s===2) dCnt++; else if(s===3) rCnt++;
                    }
                });
                
                const epMins = 30 / 60; // 30s epoch
                const tSleep = (lCnt+dCnt+rCnt)*epMins;
                const tTotal = (wCnt+lCnt+dCnt+rCnt)*epMins;
                const eff = tTotal > 0 ? (tSleep / tTotal) * 100 : 0;
            ...
            }).catch(e => console.log('Sleep fetch err', e));

        // Form Submit
        document.getElementById('configForm').onsubmit = (e) => {
            e.preventDefault();
            const data = {
                minWakeHour: parseInt(document.getElementById('cfg_minH').value),
                minWakeMinute: parseInt(document.getElementById('cfg_minM').value),
                maxWakeHour: parseInt(document.getElementById('cfg_maxH').value),
                maxWakeMinute: parseInt(document.getElementById('cfg_maxM').value),
                alarmVolume: parseInt(document.getElementById('cfg_vol').value),
                wifiSSID: document.getElementById('cfg_ssid').value,
                wifiPassword: document.getElementById('cfg_pass').value,
                apMode: document.getElementById('cfg_ap').checked,
                saveRawEEG: document.getElementById('cfg_raw').checked
            };
...
void WebDashboard::begin(UserConfig& config) {
    ...
    if (config.apMode) {
        ...
        // We do not block here to wait for connection
    }
...
void WebDashboard::setCurrentStage(SleepStage stage, const float* confs) {
    currentStage = stage;
    if (confs) {
        for (int i=0; i<NN_OUTPUT_SIZE; i++) confidences[i] = confs[i];
    }
}
...
void WebDashboard::setupRoutes() {
    ...
    server.on("/api/sleep", HTTP_GET, [this](AsyncWebServerRequest *request){
        ...
        // Return JSON array, limit to save memory if huge
        JsonDocument doc;
        JsonArray arr = doc.to<JsonArray>();
        int limit = (sleepDataCount > 200) ? 200 : sleepDataCount; // ESP memory safe
    ...
    server.on("/api/config", HTTP_GET, [this](AsyncWebServerRequest *request){
        ...
        JsonDocument doc;
        doc["minWakeHour"] = configPtr->minWakeHour;
        doc["minWakeMinute"] = configPtr->minWakeMinute;
        doc["maxWakeHour"] = configPtr->maxWakeHour;
        doc["maxWakeMinute"] = configPtr->maxWakeMinute;
        doc["alarmVolume"] = configPtr->alarmVolume;
        doc["wifiSSID"] = configPtr->wifiSSID;
        doc["wifiPassword"] = configPtr->wifiPassword;
        doc["apMode"] = configPtr->apMode;
        doc["saveRawEEG"] = configPtr->saveRawEEG;
    ...
    server.on("/api/config", HTTP_POST, [](AsyncWebServerRequest *request){
        // Do nothing, handled in body handler
    }, NULL, [this](AsyncWebServerRequest *request, uint8_t *data, size_t len, size_t index, size_t total){
        this->handleConfigPost(request, data, len);
    });
}

void WebDashboard::handleConfigPost(AsyncWebServerRequest* request, uint8_t* data, size_t len) {
    if (!configPtr) { request->send(500); return; }
...
void WebDashboard::sendWebSocketUpdate() {
